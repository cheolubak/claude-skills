...
#### ux-researcher — UX 리서처 (도전자, Sonnet)
...
#### system-engineer — 시스템 엔지니어 (도전자, Sonnet)
...
#### risk-analyst — 리스크 분석가 (도전자, Sonnet)
...
```bash
bash "$HOME/.claude/hooks/slack-team-progress.sh" "review-team" "start" "Phase 1 시작 — UX/Tech/Risk 3개 파트 병렬 토론" "<분석 대상 요약>" >/dev/null
```
...
Phase 1 완료 알림:
```bash
bash "$HOME/.claude/hooks/slack-team-progress.sh" "review-team" "complete" "Phase 1 완료 — 3개 파트 토론 결과 수합" >/dev/null
```
...
2. **Round 2 (도전자, Sonnet)**: Round 1 결과를 받아 다음을 수행합니다:
