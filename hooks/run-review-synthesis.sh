...
# Phase 2 시작 알림
bash "$HOME/.claude/hooks/slack-team-progress.sh" "$TEAM" "start" "Phase 2 시작 — 종합 판정"
...
if [ -f "$OUTPUT_DIR/synthesis.md" ]; then
  bash "$HOME/.claude/hooks/slack-team-progress.sh" "$TEAM" "finish" "전체 리뷰 완료 — 종합 판정 보고서 제출"
else
  bash "$HOME/.claude/hooks/slack-team-progress.sh" "$TEAM" "finish" "종합 판정 완료 (결과 파일 미생성 — 수동 확인 필요)"
fi
